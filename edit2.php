...
session_start();
	...
	if($num>0)
	  echo "<script>alert('此手机号已被注册，请更换手机号！');location='edit.php';</script>";
	  else
	  {
	  	$ig="$_POST[I_agree]";
	  	if($ig!=1)
	  	echo "<script>alert('请仔细阅读协议！');location='edit.php';</script>";
	  	else
	  	{
	  		/*****数据库信息更新*****/
	  ...
	  if($_POST[ed_password1] and $_POST[ed_password2]){
	if($_POST[ed_password1]!=$_POST[ed_password2])
	  {echo "<script>alert('两次输入的密码不一致，请重新输入！');location='edit.php';</script>";}
	else
	{
		
		/*$sql="select * from db_user where username='$_SESSION[name]'";
	    $re=mysql_query($sql);
        $arr=mysql_fetch_array($re);
		echo $arr[0],$arr[1],$arr[2],$arr[3],$arr[4]."<br/>";*/
		
		
		$sqlup="UPDATE Identity_information SET password='$_POST[ed_password1]' where tel='$_SESSION[tel]'";
		mysql_query($sqlup);
		/*$sql="select * from db_user where username='$_SESSION[name]'";
	    $re=mysql_query($sql);
       $arr=mysql_fetch_array($re);
		echo $arr[0],$arr[1],$arr[2],$arr[3],$arr[4]."<br/>";*/
		echo "<p align='center'>密码修改成功！<br/>";
	}
}
if($_POST[ed_tel]){
	$sqlup="update Identity_information set tel='$_POST[ed_tel]' where tel='$_SESSION[tel]'";
...
if($_POST[ed_username]){
	$sqlup="update Identity_information set username='$_POST[ed_username]' where tel='$_SESSION[tel]'";
...
if($_POST[ed_adress]){
	$sqlup="update Identity_information set adress='$_POST[ed_adress]' where tel='$_SESSION[tel]'";
