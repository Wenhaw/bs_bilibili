...
$d_time=date("Y-m-d G:i:s",$t);//提取时间 
  ...
  $vd_path= "video/" . $_FILES["up_vd"]["name"];//获取视频地址
	...
	echo "视频或封面上传失败，无法插入数据！" ;
		...
		if($_POST['v_name']){
	$sql="select * from a_video where a_v_name='$_POST[v_name]'";
	mysql_query($sql);
	$num=mysql_affected_rows();

	  $i="insert into a_video (a_v_name, a_v_pic, a_v_path, a_v_uploader, a_v_time ,a_v_tel) values('$_POST[v_name]','$pic_path','$vd_path','$row[0]','$d_time','$row[3]')";
	  mysql_query($i);
	  $num=mysql_affected_rows();
	  if($num>0){
	    echo "视频《".$_POST[v_name]."》上传成功！管理员审核通过后可在<a href='index.php'>主页</a>查看";
	   }
	  else
	  {
	    echo "视频上传失败！";
	  ...
	  echo "<script>alert('请输入作品名称!');location='up_video.php';</script>";
	  }
	    }
	   }
	   else 
	   {
	   echo "<script>alert('请登录后投稿!');location='login.php';</script>";
	   }
