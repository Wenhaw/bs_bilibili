...
<html>
	<head>
		...
		<?php 
			...
			$sql_n="SELECT COUNT(*) AS count FROM comment WHERE v_id='$v_id'"; //查询当前视频的评论数量
			$result_n=mysql_fetch_array(mysql_query($sql_n)); 
			$count=$result_n['count']; 
						
			$sql_c = "SELECT * FROM comment WHERE v_id='$v_id' ORDER BY c_id DESC " ; //评论数据读取
	...
	<body>
		...
		<div class="vd_mainBody">
			<?php echo "<h1 class='vd_title'>". $row_v[1]." </h1>"; ?>
			<div class="time"><?php echo $row_v[5];  ?></div>
			<div class="information">25.3万播放 · 1414弹幕 &nbsp;&nbsp;&nbsp;未经作者授权，禁止转载</div>
			
			<?php
				 
				$up_head = mysql_query("SELECT * FROM identity_information WHERE username = '$row_v[4]' ");//提取up主头像
				$up_head_row = mysql_fetch_array($up_head);
				?>
			<div class="up_info">
				<?php echo "<img src='$up_head_row[4]'/>"; ?>
				<div class="up_info_right">
					<span class="up_name"><?php echo $row_v[4]; ?></span><br />
					<span class="up_sign">这种鱼塘局随手摸鱼就可以了，谈不上carry</span>
					<a href="#" class="chongdian">充电</a><a href="#" class="follow">+17.1&nbsp;&nbsp;万关注</a>
				</div>
			</div>
...
<script>
    ...
    var dp = new DPlayer({
        ...
        video: {//video/E of zhang.mp4
            url: '<?php 
          echo "$row_v[3] ";//video/E of zhang.mp4
          ?>',
            pic: '<?php 
          echo "$row_v[2] ";
          ?>'
...
<!--评论-->
<div class="comment">
	<span class="pl"><?php echo $count; ?>评论</span><br />
	<span class="qbpl">全部评论</span><a href="#" class="rmpl">热门评论</a>
	<form action="video2.php" method="post">
		<?php
					if($row!=0)
						echo "<img src=".$row[4]." />";
						else
						echo "<img src='img/head.jpg' />";
					?>
		
		<textarea name="content" placeholder="请自觉遵守互联网相关的政策法规，严禁发布色情、暴力、反动的言论。"></textarea>
		<?php echo "<input type='text' name='jhsj' value= $row_v[0] style='display:none;'/>" ?>
		<input type="submit" name="record" class="f_comment" value="发布&#13;&#10;评论" />
	...
	<div class="read_comment">
		<?php
			echo "<img src=".$row_c["c_head"]." />";
		?>
		<div class="read_comment_right">
		<span class="hf_user"><?php echo $row_c["c_username"]; ?></span>
		<p class="cm_content"><?php echo $row_c["c_content"]; ?></p>
		<div class="cm_foot">
		<span class="cm_id">#<?php echo $count; ?></span>
		<time class="cm_time"><?php echo $row_c["c_time"]; ?></time>	
		</div>
		</div>
		<div style="clear:right;"></div><!--清除read_comment_right浮动-->
		<div class="line" style="margin-top:20px ;"></div>
	</div>
	<?php 
	}
